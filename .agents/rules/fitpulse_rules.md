...
- **상단 GNB 1탭 데이터 스위처 (샘플 vs 빈 상태)**:
  - `[✨ 샘플 데이터]` vs `[⟲ 신규(빈 상태)]` 세그먼트 스위처를 상시 노출하여, `window.confirm` 알럿 없이 1클릭으로 샘플 데이터와 신규 유저 빈 상태를 즉각 전환할 수 있어야 한다.
- **모바일 요약 지표 카드 2열 & 액션 버튼 세로 스택 (Media Query Leakage 방지)**:
  - 모바일 뷰포트 내부에서는 Tailwind `sm:`, `md:` 누수를 방어하여 무조건 2열 그리드(`grid-cols-2`)와 4자 이하 짧은 라벨("총 볼륨", "완주 종목", "완료 세트", "운동 시간")을 유지하고, 버튼은 세로 스택(`flex-col w-full`)으로 배치하여 좌우 짤림을 원천 차단한다.
...
## 6. Git 브랜치 및 배포 운영 원칙 (Branching & Deployment Policy)
- **`develop` 브랜치 (기본 개발 및 대화 공간)**:
  - 사용자와의 모든 기능 개발, 버그 수정, 리팩토링 및 대화는 `develop` 브랜치에서 수행한다.
  - 모든 커밋과 일상적인 푸시는 `develop` 브랜치에 진행한다.
- **`main` 브랜치 (배포 전용)**:
  - `main` 브랜치는 Netlify 실배포를 위한 프로덕션 브랜치이다.
  - 사용자가 명시적으로 **"배포해줘"**, **"배포를 위해 머지해줘"**라고 지시할 때만 `develop`을 `main`에 머지하고 푸시한다.
  - 사전 배포 지시 없이는 절대로 `main` 브랜치에 직접 작업하거나 커밋/머지하지 않는다.

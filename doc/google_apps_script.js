/**
 * ==============================================================================
 * FitPulse (핏펄스) 구글 폼 자동 생성 Google Apps Script (GAS)
 * ==============================================================================
 * 
 ...
 * 5. 실행 로그에 출력되는 [관리자 편집용 URL] 및 [응답자 제출용 URL] 확인!
 * 
 * [사용 방법 2: 웹앱(Web App) doPost REST API 방식]
 * 1. 본 코드를 붙여넣고 우측 상단 [배포] -> [새 배포] 클릭
 * 2. 유형: "웹 앱", 액세스 권한: "모든 사용자(Anyone)"로 배포
 * 3. 발급된 웹 앱 URL로 fitpulse_survey_payload.json을 POST 전송
 * ==============================================================================
 */

/**
 * 방법 1: Google Apps Script 콘솔에서 바로 실행하여 구글 폼을 1초 만에 자동 생성하는 함수
 */
function createFitPulseSurveyDirectly() {
  const payload = getFitPulseSurveyPayload();
  const result = buildGoogleFormFromPayload(payload);
  
  Logger.log("==================================================================");
  Logger.log("🎉 FitPulse 설문지가 성공적으로 생성되었습니다!");
  Logger.log("------------------------------------------------------------------");
  Logger.log("📌 폼 ID: " + result.formId);
  Logger.log("✏️ 관리자 편집용 URL: " + result.editUrl);
  Logger.log("🔗 응답자 제출용 URL: " + result.publishedUrl);
  Logger.log("==================================================================");
...
/**
 * 방법 2: 외부 HTTP POST 요청을 받아 동적으로 폼을 생성하는 Web App 엔드포인트
 */
function doPost(e) {
  try {
    let payload;
    if (e && e.postData && e.postData.contents) {
      ...
      // payload가 비어있을 경우 기본 FitPulse 설문 데이터 적용
      payload = getFitPulseSurveyPayload();
    }

    const responseData = buildGoogleFormFromPayload(payload);
...
/**
 * 공통 폼 생성 엔진: JSON Payload를 기반으로 Google Form 객체를 완벽히 빌드
 */
function buildGoogleFormFromPayload(payload) {
  ...
  // 기본 설정: 진행률 표시줄(Progress bar) 표시 및 질문 1회 응답
  ...
  if (Array.isArray(payload.questions)) {
    payload.questions.forEach((q) => {
      ...
      switch (q.type) {
        case "PAGE_BREAK": // 섹션 분할 (페이지 넘김)
          item = form.addPageBreakItem();
          break;

        case "SECTION_HEADER": // 섹션 헤더 (동일 페이지 내 소제목)
  ...
  // 3. 생성된 폼의 URL 및 ID 반환
  return {
    status: "success",
    formId: form.getId(),
    editUrl: form.getEditUrl(),           // 관리자 편집용 URL
    publishedUrl: form.getPublishedUrl()  // 응답자 배포용 URL
  };
}

/**
 * FitPulse v2.0.0 (TDS Mobile 반영) 전체 설문지 JSON Payload 정의
 */
  ...
  return {
    ...
    questions: [
      ...
      },
      {
        ...
        helpText: "1: 매우 불만족 (눈이 피로함) ~ 5: 매우 만족 (토스처럼 눈이 편안하고 세련됨)",
        min: 1,
        max: 5,
        minLabel: "매우 불만족",
        maxLabel: "매우 만족",
      ...
      },
      {
        ...
        helpText: "1: 매우 나쁨 (텍스트 잘림) ~ 5: 매우 뛰어남 (단어 단위 줄바꿈 및 2행 분리로 명확함)",
        min: 1,
        max: 5,
        minLabel: "매우 나쁨",
        maxLabel: "매우 뛰어남",
      ...
      },
      {
        ...
        helpText: "1: 매우 비효율적임 ~ 5: 글자 잘림(Truncation) 없이 종목과 세트 정보가 시원하게 읽힘",
        min: 1,
        max: 5,
        minLabel: "매우 비효율",
        maxLabel: "매우 쾌적함",
      ...
      {
        type: "SCALE",
        title: "Q13. 모든 세트 완료 및 저장 시 나타나는 '오늘 운동 완료 축하 헤더(🎉)와 4대 핵심 지표 요약 카드(총 볼륨, 완주 종목, 완료 세트, 시간)'의 성취감 피드백은 어떠했습니까?",
